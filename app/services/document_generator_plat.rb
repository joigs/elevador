...
class DocumentGeneratorPlat
  def self.generate_document(inspection_id, principal_id, revision_id, item_id, admin_id)
    ...
    revision_base = PlatRevision.find(revision_id)
    ...
    # Reglas base del grupo (RulesPlat) + anothers
    rules = RulesPlat.where(group_id: group.id).to_a
    ...
    additional_rules = anothers.map do |another|
      RulesPlat.new(
        ...
        level:    another.level,
        code:     another.code,
        ref:      another.respond_to?(:ref) ? another.ref : nil,
        group_id: group.id
      )
    end
    ...
    # Agregado de datos de revisión sin OpenStruct
    revision_struct = Struct.new(:codes, :points, :levels, :comment)
    revision = revision_struct.new([], [], [], [])

    # Construimos los arreglos de revisión a partir de los registros de PlatRevisionRulesPlat
    revision_rules = revision_base.plat_revision_rules_plats.includes(:rules_plat)

    revision_rules = revision_rules.sort_by do |rr|
      rule = rr.rules_plat
      next [Float::INFINITY] unless rule && rule.code
    ...
    revision_rules.each do |rr|
      rule = rr.rules_plat
      next unless rule

      revision.codes   << rule.code
      revision.points  << rule.point
      revision.levels  << rr.level.to_s
      revision.comment << rr.comment.to_s
    end
    ...
    if last_revision_base
      last_revision = revision_struct.new([], [], [], [])

      last_rules = last_revision_base.plat_revision_rules_plats.includes(:rules_plat)

      last_rules = last_rules.sort_by do |rr|
        rule = rr.rules_plat
        next [Float::INFINITY] unless rule && rule.code
      ...
      last_rules.each do |rr|
        rule = rr.rules_plat
        next unless rule

        last_revision.codes   << rule.code
        last_revision.points  << rule.point
        last_revision.levels  << rr.level.to_s
        last_revision.comment << rr.comment.to_s
      end
    end
    ...
    revision_nulls = RevisionNull.where(revision_id: revision_id, revision_type: 'plat')
                                 .where('point LIKE ?', '0%')

    revision_nulls_total = RevisionNull.where(revision_id: revision_id, revision_type: 'plat')
    ...
    output_path = Rails.root.join(
      'tmp',
      "Informe N°#{inspection.number}-#{inspection.ins_date&.strftime('%m')}-#{inspection.ins_date&.strftime('%Y')}-#{item_rol}.docx"
    )
    doc.commit(output_path)

    if inspectors.second
      condicion = admin.real_name == inspectors.first.real_name ||
        admin.real_name == inspectors.second.real_name ||
        (admin.email == inspectors.first.email && admin.email.present? && inspectors.first.email.present?) ||
        (admin.email == inspectors.second.email && admin.email.present? && inspectors.second.email.present?)
    else
      condicion = admin.real_name == inspectors.first.real_name ||
        (admin.email == inspectors.first.email && admin.email.present? && inspectors.first.email.present?)
    end
    ...
    rules = RulesPlat.where(group_id: group.id).ordered_by_code.drop(11)
    ...
    rules.each_with_index do |rule, _index|


        index2 = nil
        revision.codes.each_with_index do |code, idx|
          if code == rule.code && revision.points[idx] == rule.point
            index2 = idx
            break
          end
        end
        ...
        if index2
          ...
          if level121 == 'L'
            ...
            last_revision&.codes&.each_with_index do |last_code, idx|
              if last_code == rule.code &&
                last_revision.points[idx] == rule.point &&
                last_revision.levels[idx] == 'L'
    ...
    another_ruletypes.each do |ruletype|
      ...
      if matching_anothers.any?
        ...
        matching_anothers.each do |another|
          revision.points.each_with_index do |point, idx|
            if point == another.point
              matching_revision_indices << idx unless matching_revision_indices.include?(idx)
            end
          end
        end
        ...
        if matching_revision_indices.any?
          ...
          points.each_with_index do |point, idx|
            if last_revision&.points&.include?(point)
              if last_revision.levels[idx].to_s.strip == 'L'
    ...
    doc.replace(another_si)
    doc.replace(another_comentario)
    doc.replace(comentraio_final = comentarios.map(strip).join)
  ...
  def self.prepare_images_for_document(revision_id, code)
    revision_photos = RevisionPhoto.where(revision_id: revision_id, code: code, revision_type: 'plat')
