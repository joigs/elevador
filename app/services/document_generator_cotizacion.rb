...
class DocumentGeneratorCotizacion
  ...
  def self.generate_document(facturacion_id)
    ...
    begin
      ...
      intermediate_path = Rails.root.join('tmp', "cotizacion_tabla_#{SecureRandom.hex(6)}.docx")
      ...
      doc.replace('{{excel_contacto}}', contacto)
      doc.replace('{{excel_lugar}}', direccion)
      doc.replace('{{XXX}}', facturacion.number.to_s)
