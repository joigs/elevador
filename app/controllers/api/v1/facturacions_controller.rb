    ...
    class FacturacionsController < ApplicationController
      ...
      def index
        ...
        if (!year.present? && !month.present?)
          ...
          facturacions = facturacions.where("EXTRACT(YEAR FROM fecha_venta) = ?", today.year)
                                     .where("EXTRACT(MONTH FROM fecha_venta) = ?", today.month)
          convenios = convenios.where("convenios.year = ?", today.year)
                               .where("convenios.month = ?", today.month)
